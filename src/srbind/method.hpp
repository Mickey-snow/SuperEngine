...
auto do_invoke_method(serilang::Fiber& f,
                      size_t nargs,
                      size_t nkwargs,
                      T* self,
                      R (T::*pmf)(Args...),
...
auto do_invoke_method(serilang::Fiber& f,
                      ...
                      R (T::*pmf)(Args...) const,
                      const arglist_spec& spec) -> serilang::TempValue {
  return do_invoke_method(f, nargs, nkwargs, self, (R (T::*)(Args...))pmf,
                          spec);
}

template <class T, class M>
auto invoke_method(serilang::Fiber& f,
                   size_t nargs,
                   size_t nkwargs,
                   T* self,
                   M pmf,
                   const arglist_spec& spec) -> serilang::TempValue {
  return do_invoke_method(f, nargs, nkwargs, self, pmf, spec);
}
}  // namespace detail
  ...
  return gc->Allocate<serilang::NativeFunction>(
      ...
      [method, spec = parse_spec(std::forward<A>(a)...)](
        ...
        try {
          ...
          return detail::invoke_method(f, nargs, nkwargs, self, method, spec);
